...
class Graph {
    ...
    getNeighbors(point) {
        let Neighbors = this.AdjList.get(point)
        return Neighbors
    }
}
...
returnPointNeighbors = (pointID) => {
    let i = parseInt(pointID[0]*1) 
    let j = parseInt(pointID[1]*1) 
    let possibleNeighbors = [`${i + 2}${j + 1}` ,`${i +2}${j - 1}`,`${i + 1}${j + 2}`,`${i + 1}${j - 2}`,`${i - 1}${j + 2}`,`${i - 1}${j - 2}`,`${i - 2}${j + 1}`,`${i - 2}${j - 1}`]
    possibleNeighbors = possibleNeighbors.filter(Neighbor => {
        let firstDigit = parseInt(Neighbor[0]*1) 
        let secondDigit = parseInt(Neighbor[1]*1) 
        return (
            Neighbor.length === 2 &&
            firstDigit >= 0 &&
            firstDigit <= 7 &&
...
chessBoardSize = 8
chessBoard = new Graph(chessBoardSize)
for(cell of Array.from(tableCells) ) {
    chessBoard.addVertex(`${cell.id}`)
}

for (vertex of chessBoard.AdjList.keys()){
    let adjacentPoints = returnPointNeighbors(vertex) // 00 12 43 ...
    for (point of adjacentPoints) {
        chessBoard.addEdge(vertex, point)
    }
}
console.log(chessBoard)

submitBtn.addEventListener("click", (e) => {
    let path = beginDestinationSearch(e)
    visualizePath(path)
})
function delay(ms) {
...
async function visualizePath (path) { // path is an array
    console.log(path)
    for (let point of path) {
        let promise = await delay(2000)
        addKnighttoCell(point)
    }
} 

addKnighttoCell = (currTarget_id) => {
    if (curr_element != null && curr_element != undefined ) {
        curr_element.removeAttribute('style');
        curr_element.innerHTML = ''
    }
    curr_element = document.getElementById(currTarget_id) // the element clicked
...
handleClick = (e) => {
    ...
    if (e.target.tagName === "TD") {
        ...
        currTarget_id = e.target.id // id of the clicked cell
...
beginDestinationSearch = (e) => {
    ...
    console.log("L138")
    let Way = findPath(`${currTarget_id}`, `${userDestination}`, line )
    return Way
}

isDeadEnd = (point) => {
    let Neighbors = chessBoard.getNeighbors(point)
    // if visitedPoints has every component of the set Neighbors
    let flag = true
    for (let Neighbor of Neighbors) {
        if (!visitedPoints.has(Neighbor)) flag = false
    }
    return flag
}

findPath = (startPoint, end) => {
    const queue = new Queue(); // You need to implement a Queue data structure
    queue.enqueue([startPoint]); // Each element in the queue is a path from startPoint to the current point
    ...
    while (!queue.isEmpty()) {
        ...
        if (current === end) {
            ...
            return currentPath; // Return the path once the destination is found
        }

        if (!visitedPoints.has(current)) {
            visitedPoints.add(current);
            const newNeighbors = chessBoard.getNeighbors(current);
